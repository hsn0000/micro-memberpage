...
function SettingForm({details}) {
  ...
  async function submit(e) {
    ...
    }).catch( error => setErrors(error?.response?.data?.message ?? "Error"))
  }
        ...
        <div className="flex justify-start items-center -mx-5">
          <div className="w-auto text-center px-5">
            <div className="rounded-full overflow-hidden w-24 h-24">
              {
                state.avatar ? 
                <img src={state.avatar} alt="Previews" className="object-cover w-full h-full"/> :
                <DefaultUser className="fil-indigo-500" style={{width: 90, height: 90}}></DefaultUser>
              }
            </div>
          </div>
          <div className="w-full flex flex-col">
            ...
            <div>
              ...
              <button onClick={() => addPicture.current.click()} className="bg-gray-300 hover:bg-gray-400 transition-all duration-200 focus:outline-none shadow-inner text-white px-6 py-3 mt-2">
                Browse
              </button>
            </div>
          </div>
